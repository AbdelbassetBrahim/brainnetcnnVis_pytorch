...
def construct_dataset(mean_connectome,first_injury_location, second_injury_location,noise_weight=0.0625,rseed=None,nsamples=1000):
    """
    ...
    prams : 
        ...
        - noise_weight : float, level of noise
        - rseed : seed for random generator (Default : None)
        - nsamples : number of generated samples for each class
    returns : tuple (X,Y)
    ...
    S = generate_injury_signatures(X_mn=mean_connectome,r_state=r_state,sig_indexes=[first_injury_location])
    X1,_ = sample_injury_strengths(nsamples,mean_connectome,S,noise_weight)
    S = generate_injury_signatures(X_mn=mean_connectome,r_state=r_state,sig_indexes=[second_injury_location])
    X2,_= sample_injury_strengths(nsamples,mean_connectome,S,noise_weight)
    X = np.concatenate([X1,X2],axis=0)
    X = X.reshape(X.shape[0],X.shape[2],X.shape[3],1)
    Y1 = np.array([1]*nsamples)
    Y2 = np.array([0]*nsamples)
